...
interface SpeakMoreKeysShortcutInputProps {
...
export const SpeakMoreKeysShortcutInput: React.FC<
  SpeakMoreKeysShortcutInputProps
> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopSpeakmoreKeysRecording().catch(console.error);
  ...
  // Set up event listener for speakmore-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<SpeakMoreKeysEvent>(
        "speakmore-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopSpeakmoreKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startSpeakmoreKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
    ...
    >
      <div className="flex items-center space-x-1">
        {isRecording ? (
          <div
            ref={shortcutRef}
            className="rounded-md border border-logo-primary bg-logo-primary/15 px-2 py-1 text-sm font-semibold text-logo-primary"
          >
            {formatCurrentKeys()}
          </div>
        ) : (
          <div
            className="frost-control cursor-pointer rounded-md border px-2 py-1 text-sm font-semibold transition-colors hover:border-logo-primary/35 hover:text-logo-primary"
