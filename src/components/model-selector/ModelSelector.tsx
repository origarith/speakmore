...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onError }) => {
  ...
  const [cloudAsrStatus, setCloudAsrStatus] =
    useState<AsrProviderStatus | null>(null);
  const [cloudAsrStatusError, setCloudAsrStatusError] = useState(false);
  ...
  const isCloudAsrProvider = activeAsrProviderId === ALIYUN_PROVIDER_ID;
  const cloudAsrModel =
    settings?.asr_models?.[ALIYUN_PROVIDER_ID] ?? ALIYUN_DEFAULT_MODEL;
  ...
  useEffect(() => {
    if (!isCloudAsrProvider) {
      setCloudAsrStatus(null);
      setCloudAsrStatusError(false);
      return;
    }

    let cancelled = false;

    const checkCloudAsrStatus = async () => {
      setCloudAsrStatusError(false);
      const result = await commands.getAsrProviderStatus(activeAsrProviderId);
      if (cancelled) return;
      if (result.status === "ok") {
        setCloudAsrStatus(result.data);
      } else {
        setCloudAsrStatus(null);
        setCloudAsrStatusError(true);
      }
    };

    checkCloudAsrStatus().catch(() => {
      if (!cancelled) {
        setCloudAsrStatus(null);
        setCloudAsrStatusError(true);
      }
  ...
  const getModelDisplayText = (): string => {
    if (isCloudAsrProvider) {
      if (cloudAsrStatusError) {
        return t("modelSelector.cloudAsrStatusError");
      }
      if (!cloudAsrStatus) {
        return t("modelSelector.cloudAsrChecking");
      }
      if (!cloudAsrStatus.configured) {
        return t("modelSelector.cloudAsrMissingKey");
      }
      return t("modelSelector.cloudAsrReady", {
        model: cloudAsrStatus.model || cloudAsrModel || ALIYUN_DEFAULT_MODEL,
      });
    }
  ...
  const getDisplayStatus = (): ModelStatus => {
    if (isCloudAsrProvider) {
      if (cloudAsrStatusError || cloudAsrStatus?.configured === false) {
        return "error";
      }
      if (!cloudAsrStatus) return "loading";
      return "ready";
    }
    ...
    <>
      ...
      <div className="relative" ref={dropdownRef}>
        <ModelStatusButton
          ...
          isDropdownOpen={!isCloudAsrProvider && showModelDropdown}
          onClick={() => {
            if (!isCloudAsrProvider) {
              setShowModelDropdown(!showModelDropdown);
            }
          }}
          disabled={isCloudAsrProvider}
          showDropdownIndicator={!isCloudAsrProvider}
          title={
            isCloudAsrProvider
              ? t("modelSelector.cloudAsrTitle")
              : t("modelSelector.localModelTitle")
          }
        />

        {/* Model Dropdown */}
        {!isCloudAsrProvider && showModelDropdown && (
          <ModelDropdown
            models={models}
