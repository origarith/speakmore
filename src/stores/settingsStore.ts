...
interface SettingsStore {
  ...
  setAsrProvider: (providerId: string) => Promise<void>;
  updateAsrApiKey: (providerId: string, apiKey: string) => Promise<void>;
  updateAsrModel: (providerId: string, model: string) => Promise<void>;
  applyTranscriptionProfile: (profileId: string) => Promise<void>;
...
const settingUpdaters: {
  ...
  post_process_selected_preset_id: (value) =>
    commands.setPostProcessSelectedPreset(value as string),
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    updatePostProcessReasoningEffort: async (providerId, effort) => {
      const { setUpdating, refreshSettings } = get();
      const updateKey = `post_process_reasoning_effort:${providerId}`;
      ...
      try {
        ...
        console.error("Failed to update post-process reasoning effort:", error);
        throw error;
      } finally {
        setUpdating(updateKey, false);
      }
    ...
    setAsrProvider: async (providerId) => {
      const { settings, setUpdating, refreshSettings } = get();
      const updateKey = "asr_provider_id";
      const previousId = settings?.asr_provider_id ?? null;
      ...
      if (settings) {
        set((state) => ({
          settings: state.settings
            ? { ...state.settings, asr_provider_id: providerId }
            : null,
        }));
      }

      try {
        const result = await commands.setAsrProvider(providerId);
        if (result.status === "error") {
          throw new Error(result.error);
        }
        await refreshSettings();
      } catch (error) {
        console.error("Failed to set ASR provider:", error);
        if (previousId !== null) {
          set((state) => ({
            settings: state.settings
              ? { ...state.settings, asr_provider_id: previousId }
              : null,
          }));
        }
        throw error;
    ...
    updateAsrApiKey: async (providerId, apiKey) => {
      const { setUpdating, refreshSettings } = get();
      const updateKey = `asr_api_key:${providerId}`;

      setUpdating(updateKey, true);

      try {
        const result = await commands.changeAsrApiKey(providerId, apiKey);
        if (result.status === "error") {
          throw new Error(result.error);
        }
        await refreshSettings();
      } catch (error) {
        console.error("Failed to update ASR API key:", error);
        throw error;
      } finally {
        setUpdating(updateKey, false);
      }
    ...
    updateAsrModel: async (providerId, model) => {
      const { setUpdating, refreshSettings } = get();
      const updateKey = `asr_model:${providerId}`;

      setUpdating(updateKey, true);

      try {
        const result = await commands.changeAsrModel(providerId, model);
        if (result.status === "error") {
          throw new Error(result.error);
        }
        await refreshSettings();
      } catch (error) {
        console.error("Failed to update ASR model:", error);
        throw error;
      } finally {
        setUpdating(updateKey, false);
      }
    ...
    applyTranscriptionProfile: async (profileId) => {
      const { setUpdating, refreshSettings } = get();
      const updateKey = "selected_transcription_profile_id";

      setUpdating(updateKey, true);

      try {
        const result = await commands.applyTranscriptionProfile(profileId);
        if (result.status === "error") {
          throw new Error(result.error);
        }
        await refreshSettings();
      } catch (error) {
        console.error("Failed to apply transcription profile:", error);
        throw error;
      } finally {
        setUpdating(updateKey, false);
      }
