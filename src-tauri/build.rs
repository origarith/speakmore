...
fn build_apple_intelligence_bridge() {
    ...
    // sherpa-onnx's arm64 macOS prebuilts are compiled for macOS 13.4, so the
    // Swift bridge must use the same deployment floor as the Rust link step.
    let macos_deployment_target =
        env::var("MACOSX_DEPLOYMENT_TARGET").unwrap_or_else(|_| "13.4".to_string());
    let swift_target = format!("arm64-apple-macosx{macos_deployment_target}");
    ...
    let mut use_foundation_models = source_file == REAL_SWIFT_FILE;
    if !status.success() && use_foundation_models {
        println!(
...
fn link_apple_clang_runtime() {
    ...
    if runtime.ends_with("libclang_rt.osx.a") && std::path::Path::new(&runtime).exists() {
