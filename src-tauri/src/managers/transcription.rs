...
use crate::settings::{
    get_settings, ModelUnloadTimeout, OrtAcceleratorSetting, Qwen3AsrFamilySettings,
...
struct Qwen3AsrEngine {
    recognizer: OfflineRecognizer,
}

impl Qwen3AsrEngine {
    fn load(model_path: &Path, settings: &Qwen3AsrFamilySettings) -> Result<Self> {
        ModelManager::validate_qwen3_asr_model_dir(model_path)?;

        let path_string = |path: &Path| path.to_string_lossy().into_owned();
        let hotwords = qwen3_asr_hotwords(settings);
        ...
        config.model_config.qwen3_asr = OfflineQwen3ASRModelConfig {
            conv_frontend: Some(path_string(&model_path.join("conv_frontend.onnx"))),
            encoder: Some(path_string(&model_path.join("encoder.int8.onnx"))),
            decoder: Some(path_string(&model_path.join("decoder.int8.onnx"))),
            tokenizer: Some(path_string(&model_path.join("tokenizer"))),
            max_total_len: settings.max_total_len.min(i32::MAX as u32) as i32,
            max_new_tokens: settings.max_new_tokens.min(i32::MAX as u32) as i32,
            temperature: 1e-6,
            top_p: 0.8,
            seed: 42,
            hotwords,
        ...
        let recognizer = OfflineRecognizer::create(&config)
            .ok_or_else(|| anyhow::anyhow!("Failed to create Qwen3-ASR ONNX recognizer"))?;
    ...
    fn transcribe_with(
        ...
        let result = stream
            .get_result()
            .ok_or_else(|| anyhow::anyhow!("Qwen3-ASR did not return a result"))?;
...
fn qwen3_asr_hotwords(settings: &Qwen3AsrFamilySettings) -> Option<String> {
    let hotwords = settings
        .custom_vocabulary
        .iter()
        .map(|word| word.trim())
...
fn qwen3_asr_language_label(language: &str) -> Option<String> {
...
impl TranscriptionManager {
    ...
    pub fn load_model(&self, model_id: &str) -> Result<()> {
        ...
        let loaded_engine = match model_info.engine_type {
            ...
            EngineType::Qwen3Asr => {
                let settings = get_settings(&self.app_handle);
                let engine =
                    Qwen3AsrEngine::load(&model_path, &settings.asr_family_settings.qwen3_asr)
                        .map_err(|e| {
                            let error_msg =
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("SPEAKMORE_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (SPEAKMORE_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
        ...
        let selected_language = match model_info.as_ref().map(|info| &info.engine_type) {
            Some(EngineType::Whisper) => &settings.asr_family_settings.whisper.language,
            Some(EngineType::Qwen3Asr) => &settings.asr_family_settings.qwen3_asr.language,
            ...
            let is_supported = model_info
                ...
                .map(|info| {
                    info.supported_languages.is_empty()
                        || info.supported_languages.contains(selected_language)
                })
                .unwrap_or(true);
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(
                || -> Result<transcribe_rs::TranscriptionResult> {
                    match &mut engine {
                        ...
                        LoadedEngine::Qwen3Asr(qwen3_asr_engine) => qwen3_asr_engine
                            .transcribe_with(&audio, qwen3_asr_language_label(&validated_language))
        ...
        // Skip models that consume vocabulary as prompt/hotwords during inference.
        let uses_prompt_custom_vocabulary = model_info
            .as_ref()
            .map(|info| matches!(info.engine_type, EngineType::Whisper | EngineType::Qwen3Asr))
            .unwrap_or(false);

        let corrected_result =
            if !settings.custom_words.is_empty() && !uses_prompt_custom_vocabulary {
                apply_custom_words(
                    &result.text,
        ...
        let translation_enabled = model_info
            .as_ref()
            .map(|info| match info.engine_type {
                EngineType::Whisper => settings.asr_family_settings.whisper.translate_to_english,
                _ => settings.translate_to_english,
            })
            .unwrap_or(settings.translate_to_english);
        let translation_note = if translation_enabled {
            " (translated)"
        } else {
            ""
        };
