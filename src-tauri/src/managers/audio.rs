...
type RecordingChunkCallback = Arc<dyn Fn(Vec<f32>) + Send + Sync + 'static>;
type SharedRecordingChunkCallback = Arc<Mutex<Option<RecordingChunkCallback>>>;
...
fn create_audio_recorder(
    ...
    recording_chunk_callback: SharedRecordingChunkCallback,
    ...
    let recorder = AudioRecorder::new()
        ...
        .with_recording_chunk_callback(move |samples| {
            let cb = recording_chunk_callback.lock().unwrap().clone();
            if let Some(cb) = cb {
                cb(samples);
            }
        });
...
impl AudioRecordingManager {
    ...
    pub fn new(app: &tauri::AppHandle) -> Result<Self, anyhow::Error> {
        ...
        let manager = Self {
            ...
            recording_chunk_callback: Arc::new(Mutex::new(None)),
        };
    ...
    pub fn preload_vad(&self) -> Result<(), anyhow::Error> {
        ...
        if recorder_opt.is_none() {
            ...
            *recorder_opt = Some(create_audio_recorder(
                ...
                &self.app_handle,
                self.recording_chunk_callback.clone(),
            )?);
        }
        Ok(())
    }
    ...
    pub fn set_recording_chunk_callback(&self, callback: Option<RecordingChunkCallback>) {
        *self.recording_chunk_callback.lock().unwrap() = callback;
    }
    ...
    pub fn stop_recording(&self, binding_id: &str) -> Option<Vec<f32>> {
        self.stop_recording_with_raw(binding_id)
            .map(|recording| pad_short_audio(recording.processed_samples))
    }

    pub fn stop_recording_with_raw(&self, binding_id: &str) -> Option<RecordedAudio> {
        let mut state = self.state.lock().unwrap();
...
fn pad_short_audio(samples: Vec<f32>) -> Vec<f32> {
