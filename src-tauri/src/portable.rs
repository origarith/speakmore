...
const SPEAKMORE_PORTABLE_MARKER: &str = "SpeakMore Portable Mode";

// Portable mode support for SpeakMore.
//
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            // Older portable builds could leave an empty marker next to Data/.
            let _ = std::fs::write(&marker_path, SPEAKMORE_PORTABLE_MARKER);
            true
        } else {
            false
        };
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let marker = s.trim();
            marker.starts_with(SPEAKMORE_PORTABLE_MARKER)
        })
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("speakmore_test_valid");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "{SPEAKMORE_PORTABLE_MARKER}").unwrap();
    ...
    fn test_empty_file_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("speakmore_test_empty");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
    ...
    fn test_wrong_content_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("speakmore_test_wrong");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_legacy_empty_marker_without_data_dir_does_not_enable_portable() {
        ...
        let dir = std::env::temp_dir().join("speakmore_test_legacy_no_data");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();
        assert!(!is_valid_portable_marker(&marker));
    ...
    #[test]
    fn test_legacy_empty_marker_with_data_dir_enables_portable() {
        let dir = std::env::temp_dir().join("speakmore_test_legacy_with_data");
        let data_dir = dir.join("Data");
        std::fs::create_dir_all(&data_dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();

        let is_portable = if is_valid_portable_marker(&marker) {
            true
        } else if marker.exists() && data_dir.exists() {
            std::fs::write(&marker, SPEAKMORE_PORTABLE_MARKER).unwrap();
            true
        } else {
            false
        };
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        let dir = std::env::temp_dir().join("speakmore_test_ws");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        writeln!(f, "  {SPEAKMORE_PORTABLE_MARKER}").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
}
