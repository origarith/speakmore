...
pub fn switch_active_model(app: &AppHandle, model_id: &str) -> Result<(), String> {
    ...
    let active_language = {
        let selected_language = match &model_info.engine_type {
            EngineType::Whisper => &mut settings.asr_family_settings.whisper.language,
            EngineType::Qwen3Asr => &mut settings.asr_family_settings.qwen3_asr.language,
            _ => &mut settings.selected_language,
        };
        ...
        if selected_language.as_str() != "auto"
            && !model_info.supported_languages.is_empty()
            && !model_info.supported_languages.contains(&*selected_language)
        {
            log::info!(
    ...
    settings.selected_language = active_language;
...
pub async fn has_any_models_available(
    ...
    let settings = get_settings(&app_handle);
    if crate::asr::has_configured_cloud_asr(&settings) {
        return Ok(true);
    }
