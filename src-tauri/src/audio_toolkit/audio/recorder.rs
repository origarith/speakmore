...
const REALTIME_CHUNK_SAMPLES: usize = constants::WHISPER_SAMPLE_RATE as usize * 30 / 1000;

pub type RecordingChunkCallback = Arc<dyn Fn(Vec<f32>) + Send + Sync + 'static>;
...
pub struct AudioRecorder {
    ...
    recording_chunk_cb: Option<RecordingChunkCallback>,
}
...
impl AudioRecorder {
    ...
    pub fn stop(&self) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        Ok(self.stop_with_raw()?.processed_samples)
    }

    pub fn stop_with_raw(&self) -> Result<RecordedAudio, Box<dyn std::error::Error>> {
...
fn run_consumer(
    ...
    let mut raw_recorded_samples = Vec::<f32>::new();
    let mut raw_chunk_buffer = Vec::<f32>::new();
    ...
    fn handle_frame(
        ...
        out_buf: &mut Vec<f32>,
        raw_buf: &mut Vec<f32>,
        raw_chunk_buf: &mut Vec<f32>,
        chunk_cb: &Option<RecordingChunkCallback>,
    ) {
        if !recording {
            return;
        }
        ...
        if chunk_cb.is_some() {
            raw_chunk_buf.extend_from_slice(samples);
            while raw_chunk_buf.len() >= REALTIME_CHUNK_SAMPLES {
                let chunk = raw_chunk_buf
    ...
    while let Ok(chunk) = sample_rx.recv() {
        ...
        frame_resampler.push(&raw, &mut |frame: &[f32]| {
            handle_frame(
                ...
                &mut raw_recorded_samples,
                &mut raw_chunk_buffer,
                &recording_chunk_cb,
            )
        });
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            match cmd {
                Cmd::Start => {
                    ...
                    raw_recorded_samples.clear();
                    raw_chunk_buffer.clear();
                    recording = true;
                    visualizer.reset();
                    if let Some(v) = &vad {
                        v.lock().unwrap().reset();
                ...
                Cmd::Stop(reply_tx) => {
                    ...
                    if !raw_chunk_buffer.is_empty() {
                        if let Some(cb) = &recording_chunk_cb {
                            cb(std::mem::take(&mut raw_chunk_buffer));
                        }
                    }

                    let _ = reply_tx.send(RecordedAudio {
                        processed_samples: std::mem::take(&mut processed_samples),
                        raw_samples: std::mem::take(&mut raw_recorded_samples),
                    });
